...
const HealthWellnessEvents = () => {
    const [events, setEvents] = useState([]);

    useEffect(() => {
        const fetchHealthWellnessEvents = async () => {
            try {
                const response = await api.get('/api/events/events');
                // Filter for HealthWellness category
                const HealthWellnessEvents = response.data.filter(event => event.category === "Health & Wellness");
                setEvents(HealthWellnessEvents);
            } catch (error) {
                console.error("Error fetching HealthWellness events:", error);
            }
        };

        fetchHealthWellnessEvents();
    }, []);
        ...
        <section className="my-8 p-6 w-full">
            <div className="flex items-center mb-6 pl-1">
                <span className='text-red-600'>
                    <DomainAddIcon style={{ fontSize: 50 }} />
                </span>
                <h2 className="text-4xl font-semibold ml-2 text-red-600">HealthWellness Events</h2>
            </div>
...
export default HealthWellnessEvents;