    ...
    useEffect(() => {
        const fetchConcertEvents = async () => {
            try {
                ...
                // Filter for Concert category
                const ConcertEvents = response.data.filter(event => event.category === "Concert");
        ...
        <section className="my-8 p-6 w-full">
            <div className="flex items-center mb-6 pl-1">
                <span className='text-red-600'>
                    <MicExternalOnIcon style={{ fontSize: 50 }} />
                </span>
                <h2 className="text-4xl font-semibold ml-2 text-red-600">Concert Events</h2>
            </div>
